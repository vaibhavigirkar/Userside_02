...
def browse_all_projects():
    ...
    # Filter by project type (map to technology)
    if project_type:
        type_mapping = {
            'web': 'Web',
            'mobile': 'Android',
        ...
        if project_type in type_mapping and type_mapping[project_type]:
            filtered_projects = [p for p in filtered_projects if type_mapping[project_type] in p['technology']]
    ...
    if difficulties:
        filtered_projects = [p for p in filtered_projects if p['difficulty'].lower() in difficulties]
